...
void System :: initialize_velocities(){
  ...
  if(_restart){
    ...
    for (int i=0; i<_npart; i++){
      ...
      sumv(2) += vz(i);
      */
    //valido solo per il Lab4, dopo decommentare la parte sopra, per es4 voglio tutte le particelle in metà scatola
    if(i%6 == 0){vx(i) = v; vy(i) = 0.0; vz(i) = 0.0;} //inizializzo le velocità come delta di dirac
    ...
    //sumv non lo scrivo perchè farà 0
    }
...
void System :: read_configuration(){
  ...
  if(cinf.is_open()){
    ...
    for(int i=0; i<_npart; i++){
      ...
      //valido solo per il 4.2, dopo decommentare la parte sopra e commentare quella sotto, per es4.2 voglio tutte le particelle in metà scatola
      /*
      _particle(i).setposition(0, this->pbc(_side(0)*0.5*x, 0));
      _particle(i).setposition(1, this->pbc(_side(1)*0.5*y, 1));
      _particle(i).setposition(2, this->pbc(_side(2)*0.5*z, 2));
      _particle(i).acceptmove(); // _x_old = _x_new
    */
      }
...
void System :: averages(int blk){
  ...
  _average     = _block_av / double(_nsteps); //media di un blocco
  _global_av  += _average; //media tra tutte le medie dei blocchi
  ...
  if (_measure_pofv){
    ...
    for(int i=0; i<_n_bins_v; i++){
      ...
      coutf << setw(12) << (i+0.5)*_bin_size_v //bin
      //centro del bin
            << setw(12) << average
            << setw(12) << sum_average/double(blk)
            << setw(12) << this->error(sum_average, sum_ave2, blk) << endl;
