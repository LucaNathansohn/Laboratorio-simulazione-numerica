...
void System :: move(int i){ // Propose a MC move for particle i
  if(_sim_type == 3){ //Gibbs sampler for Ising
    //accetto sempre, la prob che sia su è 1-quella che sia giù
    double delta_E;
    delta_E =2.0*( _J * (_particle(this->pbc(i-1)).getspin() + _particle(this->pbc(i+1)).getspin() ) + _H );
    if (_rnd.Rannyu() < 1.0/(1.0+exp(-_beta*delta_E))){ //Gibbs acceptance evaluation for a spin flip involving spin i
  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ...
        input>>_n_bins_v;
        _nprop += _n_bins_v;
        _bin_size_v = 4.0*sqrt(_temp)/(double)_n_bins_v; // FIXED IN EXERCISE 4
        _measure_pofv = true;
...
void System :: measure(){ // Measure properties
  ...
  if (_measure_penergy or _measure_pressure or _measure_gofr) {
    ...
    for (int i=0; i<_n_bins; i++){
      _measurement(_index_gofr+i)/=(4*M_PI/3)*(pow(i*_bin_size+_bin_size,3)-pow(i*_bin_size,3))*_rho*_npart;
    }
  }
  ...
  if(_measure_pofv){
    double v;
    for(int i=0; i<_npart; i++){
      v = sqrt( dot( _particle(i).getvelocity() , _particle(i).getvelocity()));
      bin = int(v/_bin_size_v);//
      if(bin < _n_bins_v) _measurement(_index_pofv + bin) += 1.0;//measurement vettorone di grandezze
      //per velocità uso diversi elementi per riempire i bin
    }
    for(int i=0; i<_n_bins_v; i++){
      _measurement(_index_pofv + i) /= double(_npart * _bin_size_v);
    }// normalizza a 1 la probailità
  }
...
if (_measure_kenergy){
  ...
  // FIXED IN EXERCISE 6
  if (_measure_cv){
  double U= _measurement(_index_tenergy); // per il calore specifico serve l'energia totale
  _measurement(_index_cv) = U;
  _measurement(_index_cv+1) = U*U;
...
void System :: averages(int blk){
  ...
  //equivalente di global av e globalav2 per il cv
  double sum_cv=0.0;
  double sum_cv2=0.0;
  ...
  if (_measure_cv){
    ...
    double cv=_beta*_beta*(_average(_index_cv+1)-_average(_index_cv)*_average(_index_cv));
    sum_cv=_beta*_beta*(_global_av(_index_cv+1)/blk-pow(_global_av(_index_cv)/blk,2));
