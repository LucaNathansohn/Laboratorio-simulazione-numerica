...
Popolazione::Popolazione(int n_individui, int n_citta)
{
    _n_ind = n_individui;
    _n_citta = n_citta;
...
void Popolazione::calcola_fitness()
{
    ...
    for (int j=0; j < _n_ind; j++)
    {
        ...
        {
            _fitness(j) += pow(_distanze(_generazione(j).get_tragitto_component(i) - 1, _generazione(j).get_tragitto_component(i + 1) - 1),1); //metti al quadrato per fare L2, lascia così per fare L1
        }
...
{
    _ranking = sort_index(_fitness, "ascend"); // ordinamento crescente
